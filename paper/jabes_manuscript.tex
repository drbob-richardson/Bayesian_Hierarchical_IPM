...
% sections are drafted; biology/ecology pieces are flagged like this. The bold
% label here is a deliberate editorial marker, not manuscript text.
\newcommand{\needbio}[1]{%
  \par\smallskip\noindent%
  \textcolor{purple}{$\blacktriangleright$~\textbf{[COAUTHOR INPUT NEEDED, biology/ecology]:}\space\textit{#1}}%
  \par\smallskip}

\title{What Can Repeated Size Profiles Identify?\\
...
\begin{abstract}
\noindent
Repeated cross-sectional size profiles, meaning counts of individuals by body
size collected over several years, are among the most common and least expensive
data in population ecology. It is tempting to invert a size-structured integral
projection model (IPM) against such data and read off survival, growth, and
reproduction. We ask a prior question instead: which vital rates are identified
by repeated profiles, which are confounded, and what additional information
resolves them. Using an individual-based generator distinct from the recovery
model, a positive numerical-quadrature inverse IPM, and a fully Bayesian
treatment, we find that size profiles reliably recover survival, the growth
increment, and the recruit-size distribution, while recruitment intensity is only
weakly identified, along a single ridge in parameter space. Two identifiability
results explain this. A decomposition result shows that the total per-capita
contribution of each size is identified but its split into survival and
recruitment is not, wherever growth and recruit sizes overlap. An excitation
result shows that near a stable size distribution, profiles constrain the
asymptotic growth rate $\lambda$ and the stable shape far more tightly than the
vital rates that generate them. We confirm both numerically, calibrate the
posterior approximation by simulation-based calibration, quantify how
size-dependent detection corrupts recovery, and measure how much auxiliary
individual-level data are needed to break the confounding. A 20-year brook trout
capture--mark--recapture study illustrates the practical risk: a profile-only fit
can look entirely plausible yet badly misestimate growth and the size pattern of
survival, which only the individual data reveal. We therefore present the work as
a diagnostic framework for what is identified, how to detect confounding, and how
to design the data or priors needed for credible inference, rather than as a
claim that profiles generally identify vital rates.
\end{abstract}

\noindent\textit{Keywords:} integral projection models; identifiability;
...
\citep{easterling2000,ellner2006,merow2014} gives a natural mechanistic
...
We argue that the right question is not ``can we fit an inverse IPM to size
profiles'' (we almost always can) but ``what does the fit identify.'' A model can
reproduce observed profile dynamics accurately while estimating a
population-effective transition operator that differs from the individual vital
rates that generated the data, and it can report narrow posterior intervals that
are an artifact of the prior rather than the data. We therefore treat
identifiability, confounding, and prior sensitivity as objects of study in their
own right, declare the estimands before fitting, and validate recovery against
data generated by a process other than the recovery model.

This paper makes five contributions. We separate an individual-based biological
generator from the recovery model, so that the inverse IPM is never validated
only on data from the same IPM. We characterize, analytically and numerically,
which vital rates repeated profiles identify, and the confounding ridge between
recruitment intensity and its size gradient. We give a calibrated Bayesian
workflow, including simulation-based calibration of the posterior approximation
and a demonstration that size-dependent detection, if unmodeled, is read as
demography. We quantify how much auxiliary individual data break the confounding,
including robust prior borrowing and its transportability limits. Finally, we
apply the framework to a long brook trout time series, benchmarking profile-only
inference against capture--mark--recapture and integrated analyses. The methods
and diagnostics are meant to carry over to other size- and stage-structured
inverse problems.
...
develops the identifiability analysis (Section~\ref{sec:ident}), reports the
simulation experiments (Section~\ref{sec:sim}), presents the brook trout
application (Section~\ref{sec:app}), and discusses scope and limitations
...
since good performance on one does not imply recovery of another:
\begin{enumerate}
  \item \textit{Individual vital rates:} survival probability $s(x)$, mean and
  ...
  individual size $x$.
  \item \textit{Population-effective rates:} the same quantities after averaging
  over unobserved heterogeneity, movement, and detection, which is the most a
  size-only model can target.
  \item \textit{Kernel functionals:} the dominant eigenvalue $\lambda$ (asymptotic
  population growth rate), the stable size distribution, and sensitivities.
  \item \textit{Prediction targets:} next-year and multi-year size profiles.
\end{enumerate}
...
stochastic events, and the simulated population does not evolve by deterministic
...
$\int g(x'\mid x)\,dx'=\int b(x'\mid x)\,dx'=1$. The column integrals of $P$ and
$F$ are then exactly $s(x)$ and $r(x)$, which removes arbitrary scaling between
recruitment intensity and recruit-size shape. This normalization does not by
itself identify the split of $K$ into $P$ and $F$; that split must be learned
from temporal information, restrictions, priors, or auxiliary data. The
vital-rate functions are parameterized at a centering size of 80 with
$z(x)=(x-80)/20$:
\begin{align}
  ...
  \sigma_b &= e^{\eta_b},
\end{align}
giving ten demographic parameters; the Gamma--Poisson model adds $\log\phi$. The
continuous operator is discretized on a regular mesh,
$\boldsymbol{\mu}_{t+1}=\Delta x\,\mathbf{K}\boldsymbol{\mu}_t$, with conditional
normal transition probabilities integrated over target bins rather than evaluated
at bin centers, which otherwise inflates recruit-size variance at coarse bins,
and renormalized over the retained mesh.
...
Laplace approximations for the broad grids, which are fast and resumable, audited
against adaptive-Metropolis MCMC on representative datasets, and validated by
...
We report, for every fit, the prior-to-posterior contraction
$1-\mathrm{sd}_{\text{post}}/\mathrm{sd}_{\text{prior}}$ per parameter, which
separates data-informed from prior-dominated quantities; the posterior
correlation between recruitment level $\alpha_r$ and slope $\beta_r$, the leading
confounded pair; and the condition number and least-informed eigenvector of the
observed information, which expose weakly identified directions even when no
...
\smallskip
This matches what we observe. Recruit sizes are small and overlap the sizes that
small survivors grow into, so recruitment intensity is confounded with small-fish
survival and growth, while survival of larger sizes, whose grown distribution
avoids the recruit band, is identified, as is the recruit-size distribution
itself. The clearest practical consequence is the recruitment level/slope ridge
of Section~\ref{sec:sim-confound} (Figure~\ref{fig:ridge}): with all other
...
stable shape $\mathbf{w}$ are identified, while the remaining $m(m-1)$ degrees of
freedom of $\mathbf{K}$, and hence the vital rates beyond $(\lambda,\mathbf{w})$,
are not. Identifying more of $\mathbf{K}$ requires the population to occupy
multiple linearly independent states (excitation: cohort pulses, environmental
variation, or transient non-stationary dynamics).
\end{proposition}
\noindent The proposition makes precise the intuition that a population observed
near its stable size distribution carries little information about its generating
operator, and it predicts that the kernel functional $\lambda$ is recovered far
better than the rates that produce it. Section~\ref{sec:app-mcmc} bears this out
in the brook trout data, where the profile-only posterior spreads across
biologically very different vital-rate regimes that share nearly the same
$\lambda$.
...
(one long trajectory, full detection, about 6000 individuals) isolates structural
identifiability from Monte Carlo noise. The profile likelihood is sharply peaked
in most directions but nearly flat along a single ridge that trades recruitment
level against recruitment slope (Figure~\ref{fig:ridge}): the posterior
correlation between $\alpha_r$ and $\beta_r$ is $-0.96$, the curvature of the
two-dimensional log-likelihood implies a ridge correlation of $0.98$, and the
observed-information condition number exceeds $7\times10^3$. The profile data
constrain the total kernel $K$ and the influx into each size bin, but not the
partition of that influx between survival-growth flux and recruitment flux, so a
higher recruitment level with a flatter size gradient produces nearly the same
profile dynamics as a lower level with a steeper gradient.
...
along a thin ridge oriented in the negative-correlation direction, which is the
...
noise fixed (Figure~\ref{fig:excitation}). The dominant eigenvalue $\lambda$ is
recovered accurately at every excitation level (relative error below $0.01$),
including at exact stationarity, so the asymptotic functional is identified even
when the rates are not. Survival remains identified throughout (contraction about
$0.98$). Recruitment intensity, by contrast, is nearly unrecoverable near
stationarity (relative error $0.8$ to $1.1$, level/slope posterior correlation
below $-0.99$) and is progressively recovered as excitation increases, reaching
relative error $0.03$ and level/slope correlation $-0.31$ under the strongest
cohort pulse. The confounding is thus excitation-limited: a perturbed population,
following a disturbance, a strong cohort, or environmental forcing, carries the
temporal information needed to separate recruitment from survival-growth, whereas
one observed near equilibrium does not. The practical implication is a design
recommendation. To recover recruitment from size profiles, observe the population
away from its stable distribution.
...
\subsection{Robustness to flexible vital-rate forms}
\label{sec:sim-spline}
To check that the confounding is not an artifact of the parametric vital-rate
forms, we refit data generated from the parametric truth with a flexible model in
which survival, the growth increment, log-recruitment, and recruit mean are each
natural-cubic-spline functions of size (19 parameters; weakly informative, lightly
shrunk coefficient priors). The flexible model recovers survival, growth, and the
recruit-size curve closely over the data-rich size range, but fails to recover
recruitment intensity, which stays near zero and misses the true exponential rise
(Figure~\ref{fig:spline}). Averaged over sizes, the posterior coefficient of
variation of the recruitment curve is $17.5$ times that of survival ($0.57$
vs.\ $0.033$). The confounding is therefore structural, the decomposition limit of
Proposition~\ref{prop:decomp}, and not a consequence of the parametric kernel.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{spline_confounding.png}
\caption{Flexible spline inverse IPM fitted to data from the parametric truth
(dashed lines), with 95\% posterior bands. Survival (a), growth (b), and recruit
size (d) are recovered over the data-rich size range; recruitment intensity (c)
is not, remaining near zero with a wide band. The confounding persists
nonparametrically.}
\label{fig:spline}
\end{figure}
...
quantities. Recovery was strongly quantity-dependent
...
eigenvalue were biased and overconfident, the simulation counterpart of the ridge
in Section~\ref{sec:sim-confound}.
...
fits succeeded and 10 of 11 parameters had uniform rank statistics, with only the
overdispersion parameter $\log\phi$ mildly miscalibrated. Under a diffuse prior
the rank histograms became U-shaped, indicating overconfident intervals, most
severely along the confounded recruitment and growth directions. The practical
implication is that, under diffuse priors, interval inference for the confounded
directions should come from full posterior sampling rather than the Gaussian
approximation.
...
detectability is absorbed as demographic change. This is the main practical
threat to profile-only inference, and it motivates the caveats in the
application.
...
recruitment-at-80 coverage from 0.53 to 0.87. A precise but biased external study
produced low point-estimate error yet zero coverage; robust (mixture) borrowing
mitigated but did not eliminate the damage, and could reject a non-transportable
study only when the conflict altered profile dynamics along a likelihood-informed
direction (Figure~\ref{fig:transport}). No single amount of external-study bias is
universally safe.
...
On 200 datasets each carrying both profiles and capture--recapture histories, we
compared profile-only, capture--recapture-only, integrated, and complete-history
oracle analyses (Figure~\ref{fig:paired}). Capture--recapture efficiently
identified survival and growth but carried essentially no information about
recruitment; its apparently perfect recruitment coverage reflected prior-width
intervals, not recovery. Integration improved point accuracy for survival and
growth but narrowed intervals below nominal, as residual discrepancy between
observed and deterministically projected profiles was absorbed by other
parameters, a point we return to in the application.
...
The profile-only fit is internally plausible (moderate survival, positive growth,
$\lambda\approx 1$, a positive-definite posterior), yet it is contradicted by the
individual data on both quantities the recaptures can check
(Table~\ref{tab:wb}, Figure~\ref{fig:wb}). It underestimates the growth increment
(12.6 vs.\ 32.3~mm\,yr$^{-1}$), the signature of a near-stationary size
distribution that hides how fast individuals move through it, and it misses the
size gradient in apparent survival (roughly flat near 0.44, against a clear
decline from 0.31 to 0.12 with size in the recapture and integrated analyses,
reflecting greater emigration of larger fish). Recruitment intensity remains
unidentified by profiles (level/slope posterior correlation $-0.995$), as in the
simulations. The integrated analysis adopts the recapture survival and growth and
tightens the population growth rate to $\lambda = 1.016\,[1.00, 1.05]$.

\begin{table}[H]
\centering
\caption{West Brook brook trout: posterior median [95\% interval] by analysis.
Mark--recapture survival is apparent survival, which folds in size-dependent
emigration from the network.}
\label{tab:wb}
\small
\begin{tabular}{lccc}
\toprule
...
$\lambda$ & 0.99 [0.97, 1.11] & n/a & 1.016 [1.00, 1.05]\\
\bottomrule
\end{tabular}
\end{table}
...
chains) exposes the non-identifiability directly. The chains do not converge on
the vital rates (split $\widehat R$ up to $6.6$), which is a property of the
target rather than a sampler failure. They settle in biologically distinct
regimes (survival at 80\,mm from $0.005$ to $0.12$, growth increment from $3$ to
$14$\,mm, recruitment over more than an order of magnitude) that all reproduce the
near-stationary profiles of Figure~\ref{fig:wb-profiles}. Across the pooled
posterior, however, the dominant eigenvalue varies by only 2.5\% (coefficient of
variation), against 222\% for survival at 80\,mm and 131\% for recruitment
intensity (Figure~\ref{fig:manifold}), as Proposition~\ref{prop:excite} predicts.
The Laplace mode (survival about 0.43) does not lie where the posterior mass
concentrates, so its narrow intervals for the confounded directions are not
trustworthy. We therefore report $\lambda$ as the profile-identified functional
and treat profile-only vital-rate point estimates as non-identified absent
individual data; the integrated analysis both resolves the rates and shifts
$\lambda$ to $1.016\,[1.00,1.05]$.
...
certifies practical identifiability at a single point; establishing global
...
\subsection{Kernel functionals: the population is near its stable distribution}
\label{sec:app-stable}
Although the vital rates are not identified, the kernel functionals are. The
profile-only posterior gives a well-determined asymptotic growth rate
($\lambda = 0.978\,[0.958, 1.034]$) and stable size distribution: the mean
per-bin posterior coefficient of variation of the stable distribution is $0.13$,
roughly seventeen times tighter than that of survival. Both $\lambda$ and the
stable shape are pinned down across the iso-$\lambda$ manifold, as
Proposition~\ref{prop:excite} predicts. The model-implied stable distribution
closely matches the observed average size profile over the bulk of the size range
(Figure~\ref{fig:stable}), confirming that the fitted kernel is self-consistent
and that the population is observed near its stable distribution, the regime in
which the vital rates are non-identified. The one systematic departure, at the
smallest observed sizes, is consistent with under-detection of small fish near the
lower window edge. This points to the constructive reading of the analysis: from
size profiles one should report and trust the kernel functionals ($\lambda$ and
the stable distribution), while the underlying vital rates require individual
data.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{west_brook_stable.png}
\caption{Model-implied stable size distribution (median and 95\% band) from the
West Brook profile-only posterior, against the observed average size profile.
The close match over the bulk of the range shows the population is near its
stable distribution; the departure at the smallest sizes is consistent with
under-detection near the lower observation window edge.}
\label{fig:stable}
\end{figure}

\subsection{External validation against published estimates}
\label{sec:app-validation}
The West Brook system has been studied with purpose-built capture--recapture and
matrix models \citep{letcher2007,letcher2015,kanno2014}, giving an external
benchmark independent of our analysis (Table~\ref{tab:validation}). Two
differences must be reconciled. The first is time step: the published mean
seasonal survival of $0.76$ \citep{letcher2015}, compounded over four seasons,
implies an annual survival near $0.33$, close to our annual apparent survival of
$0.28$, and the published monthly survivals of $0.89$--$0.94$ \citep{letcher2007}
bracket the same range. The second, and more important, is that the published
models separate permanent emigration from mortality using boundary antennas, so
their ``survival'' is close to true survival, whereas our mark--recapture estimate
is apparent survival that confounds death with emigration. Ours is therefore
expected to be modestly lower, by roughly the emigration rate (lifetime
mainstem--tributary movement of 28--33\%; \citealp{kanno2014}). Our finding that
apparent survival declines with size matches the consistently negative size effect
on seasonal survival in \citet{letcher2015}, plausibly amplified because larger
fish both die and emigrate more. Our integrated growth (about 32~mm\,yr$^{-1}$ at
80~mm) is consistent with the spring-dominated seasonal increments of
\citet{letcher2015}; our annual detection ($0.97$) is what four seasonal occasions
at the published per-occasion $p=0.57$ would produce; and our integrated
$\lambda = 1.016$ is close to the only published estimate, $\lambda = 1.0008$
\citep{letcher2007}. The profile-plus-recapture analysis thus recovers the
externally validated demography of this population.

\begin{table}[H]
\centering
\caption{External validation of the West Brook estimates against the published
literature. Published survival is true survival (emigration modelled
separately); ours is apparent survival, expected to be modestly lower.}
\label{tab:validation}
\small
\begin{tabular}{lll}
\toprule
Quantity & This study & Published (West Brook)\\
\midrule
Annual survival & 0.28 (apparent) & $\approx$0.33 from seasonal 0.76 \citep{letcher2015};\\
 & & monthly 0.89--0.94 \citep{letcher2007}\\
Survival vs.\ size & declining, 0.31$\to$0.12 & negative size effect \citep{letcher2015}\\
Growth incr.\ @ 80 mm & 32 mm\,yr$^{-1}$ & 14 mm/season, spring-dominated \citep{letcher2015}\\
Detection & 0.97 (annual) & 0.57 per occasion $\Rightarrow$ 0.97 over 4 \citep{letcher2015}\\
$\lambda$ & 1.016 [1.00, 1.05] & 1.0008 [0.990, 1.012] \citep{letcher2007}\\
\bottomrule
\end{tabular}
\end{table}
...
Repeated size profiles are informative about some vital rates (survival, the
growth distribution, and recruit size) and almost uninformative about others,
...
detection. The brook trout application shows what is at stake: a profile-only fit
that passes every internal check still misstates growth by a factor of two and
inverts the survival size pattern, errors visible only against individual data.
...
routine identifiability diagnostics; modeling, or at minimum bounding,
size-dependent detection; and expressing external information as an explicit,
transportability-checkable study design. Where individual data exist, an
integrated analysis is worth far more than its marginal cost, though its intervals
...
separately), and a single applied system. The framework and diagnostics
themselves are general to size- and stage-structured inverse problems.
...
\bibitem[Kanno et al.(2014)]{kanno2014} Kanno, Y., Letcher, B.H., Coombs, J.A.,
Nislow, K.H., Whiteley, A.R. (2014). Linking movement and reproductive history of
brook trout to assess habitat connectivity in a heterogeneous stream network.
\emph{Freshwater Biology} 59, 142--154. \doi{10.1111/fwb.12254}.

\bibitem[Letcher et al.(2007)]{letcher2007} Letcher, B.H., Nislow, K.H., Coombs,
J.A., O'Donnell, M.J., Dubreuil, T.L. (2007). Population response to habitat
fragmentation in a stream-dwelling brook trout population. \emph{PLoS ONE} 2,
e1139. \doi{10.1371/journal.pone.0001139}.
...
\emph{Journal of Animal Ecology} 84, 337--352. \doi{10.1111/1365-2656.12308}.
