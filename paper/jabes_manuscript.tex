...
To rule out that the confounding is an artifact of the parametric vital-rate
...
natural-cubic-spline functions of size (19 parameters; weakly informative,
lightly shrunk coefficient priors). The flexible model recovers survival, growth,
and the recruit-size curve closely over the data-rich size range, but fails to
recover recruitment intensity, which stays near zero and misses the true
exponential rise (Figure~\ref{fig:spline}). Averaged over sizes, the posterior
coefficient of variation of the recruitment curve is $17.5\times$ that of survival
($0.57$ vs.\ $0.033$). The confounding is therefore structural---the decomposition
limit of Proposition~\ref{prop:decomp}---and not a consequence of the parametric
kernel.
...
and that the population is observed near its stable distribution---precisely the
regime in which the vital rates are non-identified. The one systematic departure,
at the smallest observed sizes, is consistent with under-detection of small fish
near the lower window edge. This is the constructive reading of the analysis:
from size profiles one should report and trust the kernel functionals ($\lambda$
and the stable distribution), while the underlying vital rates require individual
data.
...
differences must be reconciled. First, time step: the published mean seasonal
survival of $0.76$ \citep{letcher2015}, compounded over four seasons, implies an
annual survival near $0.33$, close to our annual apparent survival of $0.28$, and
the published monthly survivals of $0.89$--$0.94$ \citep{letcher2007} bracket the
same range. Second, and more important, the published models separate permanent
emigration from mortality using boundary antennas, so their ``survival'' is close
to true survival, whereas our mark--recapture estimate is \emph{apparent} survival
that confounds death with emigration; ours is therefore expected to be modestly
lower, by roughly the emigration rate (lifetime mainstem--tributary movement of
28--33\%; \citealp{kanno2014}). Our finding that apparent survival declines with
size matches the consistently negative size effect on seasonal survival in
\citet{letcher2015}, plausibly amplified because larger fish both die and emigrate
more. Our integrated growth ($\approx 32$~mm\,yr$^{-1}$ at 80~mm) is consistent
with the spring-dominated seasonal increments of \citet{letcher2015}; our annual
detection ($0.97$) is what four seasonal occasions at the published per-occasion
$p=0.57$ would produce; and our integrated $\lambda = 1.016$ closely matches the
only published estimate, $\lambda = 1.0008$ \citep{letcher2007}. The
profile-plus-recapture analysis thus recovers the externally validated demography
of this population.
